    ...
    <div class="app-shell">
      <aside class="sidebar">
        ...
        <nav class="nav-list" aria-label="工作台导航">
          <a class="active" href="#products">上传截图</a>
          <a href="#profiles">UX 纬度分析</a>
          <a href="#report">输出报告</a>
        </nav>
      </aside>
      ...
      <main class="workspace" id="workspace">
        ...
        ${state.health?.apiAvailable ? `
          <button class="status-panel config-strip" data-action="open-config" type="button">
            <span class="status-dot ${modelConfigured && state.health?.visionAvailable !== false ? "ok" : "warn"}"></span>
            <div>
...
function renderProduct(product) {
  const roleLabel = product.role === "self" ? "本品" : `竞品 ${competitorIndex(product.id)}`;
    ...
    <article class="product-card" data-id="${product.id}">
      <div class="capture-head">
        <div class="capture-name-row">
          <span class="capture-dot ${product.role === "self" ? "self" : "competitor"}"></span>
...
function renderRadarConclusion(report) {
  ...
  const summary = buildRadarSummary(report, radar);
  ...
  return `
    ...
    <div class="radar-layout">
      ...
      <div class="radar-copy">
        ...
        <p class="best-conclusion">${escapeHtml(report.bestConclusion)}</p>
        ${summary ? `<p class="radar-insight">${summary}</p>` : ""}
...
function renderProductAnalysisCard(productAnalysis, report) {
  ...
  const accent = productColor(productAnalysis.productName, report);
    ...
    <svg class="radar-chart" viewBox="0 0 ${size} ${size}" role="img" aria-label="各产品 UX 维度雷达图">
      ...
      <g class="radar-labels">
        ${radar.dimensions.map((dimension, index) => {
          const end = axisEnd(index);
          return `<text x="${end.labelX}" y="${end.labelY}" text-anchor="middle">${escapeHtml(dimension)}</text>`;
        }).join("")}
      </g>
    </svg>
  `;
...
function buildRadarSummary(report, radar) {
  if (!radar.products.length || !radar.dimensions.length) return "";
  const leader = report.rankings[0];
  const leaderRadar = radar.products.find((product) => product.name === leader.productName) || radar.products[0];
  const maxIndex = leaderRadar.scores.indexOf(Math.max(...leaderRadar.scores));
  const minIndex = leaderRadar.scores.indexOf(Math.min(...leaderRadar.scores));
  return `${escapeHtml(leaderRadar.name)} 在 ${escapeHtml(radar.dimensions[maxIndex])} 维度表现最强，综合 AI 评分 ${leader.score}；后续建议优先补齐 ${escapeHtml(radar.dimensions[minIndex])} 的体验短板。`;
}
    ...
    <div class="modal-backdrop" data-action="close-config">
      <section class="config-modal" role="dialog" aria-modal="true" aria-label="模型配置" data-modal-panel>
        ...
        <div class="modal-form">
          <label>
            <span>模型服务</span>
            <select data-config-field="provider">
              ${["OpenAI", "DeepSeek", "自定义"].map((provider) => `<option value="${provider}" ${state.modelConfig.provider === provider ? "selected" : ""}>${provider}</option>`).join("")}
            </select>
          </label>
          <label>
            <span>模型</span>
